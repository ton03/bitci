...
<section><h2>Recent jobs</h2><table><thead><tr><th>Job</th><th>State</th><th>Stage</th><th>Tested SHA</th><th>Queue wait</th><th>Stage duration</th><th>Timeout</th><th>Resources</th><th>Log</th></tr></thead><tbody>{{range .Jobs}}<tr><td class="num">{{.ID}}</td><td><span class="{{.State}}">{{.State}}</span></td><td>{{.Task}}</td><td><span class="sha" title="{{.TestedSHA}}">{{if .TestedSHA}}{{.TestedSHA}}{{else}}—{{end}}</span></td><td class="num">{{.QueueWait}}</td><td class="num">{{.StageDuration}}</td><td class="num">{{.Timeout}}</td><td>{{.Resources}}</td><td>{{if .LogPath}}<a href="/jobs/{{.ID}}/logs">last 80 lines</a>{{else}}—{{end}}</td></tr>{{else}}<tr><td colspan="9" class="muted">No jobs yet. Submit configured task IDs from the CLI or MCP.</td></tr>{{end}}</tbody></table></section></body></html>`))
